...
/**
 * Class CacheController 地区 执行操作控制器
 * @package backend\controllers
 */
class CacheController extends \common\controllers\UserController
{
    // 引入json 返回处理类
    use \common\traits\Json;

    use \common\traits\Alert;
    /**
     * @inheritdoc
     */
    ...
    public function actionFlushCache($id)
    {
        if ($this->getCache($id)->flush()) {
            $this->alertSuccess(\Yii::t('backend', 'Cache has been successfully flushed'));
        };
    ...
    public function actionFlushCacheKey($id, $key)
    {
        if ($this->getCache($id)->delete($key)) {
            $this->alertSuccess(\Yii::t('backend', 'Cache entry has been successfully deleted'));
        };
        return $this->redirect(['index']);
    }
    ...
    public function actionFlushCacheTag($id, $tag)
    {
        ...
        $this->alertSuccess(\Yii::t('backend', 'TagDependency was invalidated'));
        return $this->redirect(['index']);
    }
}
