...
{
    const EVENT_UEDITOR_AFTER_UPLOAD = 'afterUpload';
    ...
    public function init()
    {
        ...
        $this->webroot = Yii::getAlias('@storageUpload');
        if (!is_array($this->thumbnail))
            $this->thumbnail = false;
    }

    protected function afterUpload($result){
        $this->trigger(self::EVENT_UEDITOR_AFTER_UPLOAD, new AfterUploadEvent(['file' => $result]));
    }
    /**
     * 蛋疼的统一后台入口
     */
    ...
    protected function upload($fieldName, $config, $base64 = 'upload')
    {
        ...
        if (($this->thumbnail or $this->zoom or $this->watermark) && $info['state'] == 'SUCCESS' && in_array($info['type'], ['.png', '.jpg', '.bmp', '.gif'])) {
            $info['thumbnail'] = $baseUrl . $this->imageHandle($info['url']);
        }
    ...
    protected function imageHandle($file)
    {
        ...
        //最后生成水印
        if (isset($this->watermark['path']) && file_exists($this->webroot.$this->watermark['path'])) {
            $this->watermark['path'] = $this->webroot.$this->watermark['path'];
